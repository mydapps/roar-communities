...
const Index = () => {
  ...
  const [deviceInfoLogged, setDeviceInfoLogged] = useState(false);

  const isInviteRoute = location.pathname.includes('/invite/');

  // Only log device detection once when complete
  useEffect(() => {
    if (!deviceDetectionLoading && !deviceInfoLogged) {
      setDeviceInfoLogged(true);
      console.log('Device detection completed - isMobileApp:', isMobileApp);
    }
  }, [deviceDetectionLoading, isMobileApp, deviceInfoLogged]);

  // Stabilize the view to prevent flickering
  useEffect(() => {
    // Only stabilize the view once device detection is complete
    if (!deviceDetectionLoading) {
      // Add a small delay to ensure all detection logic is fully settled
      const timer = setTimeout(() => {
        setIsViewStabilized(true);
        console.log('Index: View stabilized with isMobileApp =', isMobileApp);
      }, 200);
  ...
  useEffect(() => {
    ...
    if (userId) {
      ...
      // Only redirect to feed if fully registered
      if (registered === "1") {
        navigate('/feed');
        return;
      }
  ...
  // NEW IMPROVED forceWebView function for determining which view to show
  const forceWebView = () => {
    // Skip this check in development mode to allow testing
    if (process.env.NODE_ENV === 'development') {
      const urlParams = new URLSearchParams(window.location.search);
      if (urlParams.get('mobileapp') === '1') {
        return false; // Don't force web view if test parameter is present
      }
    }

    // We'll rely on the isMobileApp flag which is now based directly on platform
    // No need for additional detection logic here
    return false;
  };

  // Mobile app specific render function
  const renderMobilePage = () => {
    // Dynamic URL and title/description based on invite route
    ...
    return (
      <>
        <Helmet>
          ...
          <meta name="keywords" content="social platform, censorship resistant, crypto rewards, web3, blockchain, community investment, ETH rewards, decentralized communities" />
  ...
  // Web specific render function (both desktop web and mobile web)
  const renderWebPage = () => {
    // Dynamic URL and title/description based on invite route
    const baseUrl = 'https://dapps.co';
    const currentUrl = isInviteRoute && code ? `${baseUrl}/invite/${code}` : baseUrl;
                ...
                {process.env.NODE_ENV === 'development' && isMobileApp && (
                  ...
                  >
                    <p className="font-mono">
                      <strong>Platform:</strong> {isMobileApp}
                    </p>
                  </motion.div>
                )}
  ...
  // Now render the appropriate view after stabilization
  if (!isViewStabilized) {
    return (
      <div className="flex h-screen w-screen items-center justify-center">
        <div className="text-center">
          <div className="mb-4 animate-spin text-2xl">⚙️</div>
  ...
  const shouldShowMobileApp = checkIfMobileApp() && !forceWebView();
  
  // Now render the appropriate view
  if (shouldShowMobileApp) {
    // Render mobile app version
