...
export const Post = ({ 
  ...
  // Share menu - reimagined as a sliding drawer
      ...
      <CardFooter className="py-3">
        <div className="flex justify-between w-full">
          <Button 
            variant="ghost" 
            size="sm"
            className={`flex gap-1.5 items-center group ${roared ? "text-amber-500" : ""} transition-all duration-200`}
            onClick={handleRoar}
          >
            <div className={`relative ${roarAnimation ? "animate-roar" : ""}`}>
              <span className={`text-lg transition-transform ${roared ? "text-amber-500" : ""} group-hover:scale-110`} role="img" aria-label="lion">🦁</span>
              {roarAnimation && (
          ...
          <div className="relative">
            ...
            {shareMenuOpen && (
              <div className="fixed inset-0 bg-black/20 backdrop-blur-sm z-50 flex items-end justify-center animate-fade-in" onClick={() => setShareMenuOpen(false)}>
                <div 
                  className="bg-card w-full max-w-md rounded-t-xl shadow-xl animate-slide-up border-t border-x border-border"
                  onClick={(e) => e.stopPropagation()}
                >
                  <div className="p-4 flex items-center justify-between border-b">
                    <h3 className="font-medium">Share this post</h3>
                    <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full" onClick={() => setShareMenuOpen(false)}>
                      <X className="h-4 w-4" />
                    </Button>
                  </div>
                  <div className="grid grid-cols-3 gap-2 p-4">
                    <Button variant="outline" className="flex flex-col h-20 gap-1 items-center justify-center" onClick={() => handleShare('Twitter')}>
                    ...
                    <Button variant="outline" className="flex flex-col h-20 gap-1 items-center justify-center" onClick={() => handleShare('WhatsApp')}>
                    ...
                    <Button variant="outline" className="flex flex-col h-20 gap-1 items-center justify-center" onClick={() => handleShare('Farcaster')}>
                    ...
                    <Button variant="outline" className="flex flex-col h-20 gap-1 items-center justify-center" onClick={() => handleShare('Copy Link')}>
